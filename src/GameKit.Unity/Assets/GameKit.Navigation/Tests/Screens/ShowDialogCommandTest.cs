...
namespace GameKit.Navigation.Tests.Screens
{
    public class ShowDialogCommandTest
    {
        ...
        public IEnumerator ShowDialogCommand_존재하지않는다이얼로그_DialogErrorCommand발행() => UniTask.ToCoroutine(async () =>
        {
            ...
            DialogErrorCommand? receivedError = null;
            using var subscribe = router.Subscribe<DialogErrorCommand>((error, ctx) => { receivedError = error; });
            ...
            // DialogErrorCommand 검증
            Assert.IsNotNull(receivedError);
            Assert.AreEqual("NonExistentDialog", receivedError.Value.DialogId);
            Assert.AreEqual(DialogOperation.Show, receivedError.Value.Operation);
